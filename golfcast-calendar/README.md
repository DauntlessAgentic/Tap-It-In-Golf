...
```bash
cd golfcast-calendar && npm install && cp .env.example .env
...
30 5 * * * cd /path/to/golfcast-calendar && /usr/local/bin/npm run sync:golfcast >> /tmp/golfcast.log 2>&1
```
...
<plist version="1.0">
  <dict>
    ...
    <key>WorkingDirectory</key><string>/path/to/golfcast-calendar</string>
...
jobs:
  sync:
    ...
    defaults:
      run:
        working-directory: golfcast-calendar
    steps:
